...
| `ALLOWED_ORIGINS` | comma-separated URL list | Yes | none | `http://localhost:6041` (dev) or `https://app.example.com,https://staging.example.com` (prod) | Comma-separated allow-list of UI origins. CORS requests whose `Origin` is not a member of the list are rejected. A single URL (no comma) is a one-element list. Each entry is trimmed; trailing slash and default ports (`:80` for http, `:443` for https) are normalised out at startup; every entry must be a valid URL and the list must be non-empty. The legacy singular `ALLOWED_ORIGIN` is still read as a fallback when `ALLOWED_ORIGINS` is unset (deprecated). |
| `PORT` | integer | No | `6040` | `6040` | Port the API listens on. Must be in range `6040–6049` (project constraint). Validates at startup. |
...
8. **Verify health**
   ```bash
   curl http://localhost:6040/api/health
   ```
