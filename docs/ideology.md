...
> in values.
>
> asp is a reusable SPA template with a working reference implementation. It teaches
> by example: the code that ships IS the lesson. The disciplines below are stricter
> than what most downstream forks will retain — that asymmetry is deliberate. A rigorous
> reference can be relaxed by a fork; a slack reference cannot be tightened.
...
- **We prefer a locked stack over an open template**, because the consistency between
  asp, aab, radar, and future projects in this family IS the lesson. Stack divergence
  in a reference template dilutes its teaching value.
...
BA already does correctly and introduces drift between asp's model and BA's evolving
defaults. DB-backed sessions trade Redis pub/sub for one fewer infra dependency —
a template should pin minimum infrastructure.

**Override path:** No override for BA ownership of identity. A downstream fork that
needs Redis-backed sessions (for scale or pub/sub) may swap; asp itself does not.
...
in every environment from Phase 1. Rotation procedure is documented in
`docs/architecture.md` and includes guidance to rotate on each production deploy
...
a development-grade secret reaches production. Validating from Phase 1 — before BA
is even wired — establishes the stub pattern and the env-file discipline early.
...
**Rule:** asp's reference demonstrates both auth models. System-level controls
...
predicates. Each story spec that touches an auth-gated resource names the resource
type's governing model before build.
...
**Rationale:** See `~/.claude/policies/auth-coexistence.md`. Single-tenant multi-user
apps almost always need both; conflation is the most common failure mode.
...
`routes/` and any future agent-tool layer never import from `db/`. `services/` never
imports from `routes/`. `process.env` is read only in `api/src/lib/config.ts`,
`api/src/db/index.ts`, and `drizzle.config.ts`. All other code imports the typed
config object.
...
no Redis (n/a for asp), no network. `api/src/index.ts` is the process entry point
only: it calls `buildApp()`, verifies DB and storage connectivity, registers OS
signal handlers, and calls `listen()`. `index.ts` may contain no other logic.
...
**Rule:** `/api/src/db/schema/` is modified only when a story explicitly names a
schema change. Every schema change generates a drizzle-kit migration AND adds an
ADR-style note to `docs/architecture.md` explaining the design decision. The
`.claude/settings.json` deny list blocks unscoped writes; the reviewer enforces
the ADR note.
...
is transferable to a non-TS server. asp prefers the transferable lesson.
...
CHECK constraint. The two bounds must agree; the reviewer flags drift.
...
(Phase 12, INFRA-026: `docker-compose.yml` contains `asp` only; no `postgres` service;
DB is the operator's network instance at all times.)
...
test execution only; (2) production-deployment `docker-compose.yml` that is app-layer-only
with external DB (Phase 12 INFRA-026 sign-off, 2026-05-29).

---

### Pinned dev port range 6040–6049

**Rule:** All asp dev servers (API, UI, future services) bind ports in 6040–6049.
The mapping is recorded in `docs/architecture.md`. No asp dev process uses a port
outside this range.

**Protects:** Collision-free dev environment when asp runs alongside radar
(6010–6019) and other projects in this family.

**Rationale:** Port collisions waste time; a pinned range is one line of
configuration to set and zero lines of debugging.

**Override path:** No override within asp. Stub values in non-connecting scripts
(e.g. OpenAPI generation scripts) may reference localhost with arbitrary ports
because they never open a real connection.
...
the local UI origin (e.g. `http://localhost:6041`). The config layer exits if it
is unset or empty. The legacy singular `ALLOWED_ORIGIN` is read as a fallback
(deprecated).

**Protects:** CORS awareness from Phase 1; production deployments that forget to
...
configuration. No inline hex literals. No raw CSS custom properties. The reviewer
flags any hex string in a component file.
...
`mutationFn` is `ui/src/lib/apiFetch.ts` (UI-093) — the canonical fetch wrapper that
all hooks call; raw `fetch()` to API URLs no longer appears in `ui/src/hooks/`. This
...
QueryClient default options, which trigger the re-auth flow. Server state (anything that round-trips the API) lives
in React Query's cache. Client state (modal open/closed, UI mode, ephemeral
selections) lives in Zustand. A Zustand store never duplicates data that React
Query already owns.

**Protects:** Cache-coherence bugs from dual-stored server state, single 401 handling
path, single auth-failure UX.

**Rationale:** The two-store split (radar's `store/`/`state/` pattern) fights a
synchronisation problem React Query already solves. Using each tool for its
intended job removes the problem.
...
**Rationale:** asp's purpose is to be a working reference. A non-working reference
is worse than no reference at all, because it looks authoritative.
...
**Rule:** asp's stack is React 19 + Vite + TypeScript + Tailwind 4 (UI), Fastify +
Drizzle ORM + Better Auth + Zod (API), Vitest + Playwright (tests), pnpm workspaces.
Downstream forks may diverge; asp itself does not.

**Protects:** Cross-project consistency between asp, aab, radar, and future
projects in this family. The lesson the template teaches is partly the stack.
...
**Override path:** No override within asp. Forks substitute freely.

---

## Prototype fingerprints

_(No prototype fingerprints recorded — asp is being built fresh against this
ideology. Record fingerprints during or after the first phase when patterns that
reflect developer habit (rather than the convictions above) become identifiable.)_
...
- **The 64-char SESSION_SECRET floor** — radar uses 32. The doubled minimum is
  cheap insurance, but if it produces friction (e.g. operator pushback on
...
- **Hard remote-only with no local Docker** — this is the most opinionated stance
  ...
  is a docker-compose fallback. asp commits to the harder rule; forks may not.
- **TanStack Query as the sole HTTP seam** — radar's hand-rolled `apiFetch` has
...
When two implementations of asp are compared against this ideology, these are the
