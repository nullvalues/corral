 ...
 *   ALLOWED_ORIGIN     — required for the API server (default http://localhost:6041)
 ...
 *   API_BASE           — passed to seed:uat (default http://localhost:6040)
 *   BASE_URL           — Playwright base URL (default http://localhost:6041)
 *
 * Exit-code contract:
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const playwrightEnv: NodeJS.ProcessEnv = {
      ...
      BASE_URL: childEnv['BASE_URL'] ?? 'http://localhost:6041',
      API_BASE: childEnv['API_BASE'] ?? 'http://localhost:6040',
