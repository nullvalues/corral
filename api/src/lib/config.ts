 ...
 *
 * Both `http://localhost:6041/` and `http://localhost:6041` resolve to the
...
const envSchema = z
  .object({
    ...
    PORT: z.coerce
      .number()
      .int()
      .min(6040, 'PORT must be in the asp dev range 6040–6049')
      .max(6049, 'PORT must be in the asp dev range 6040–6049')
      .default(6040),
    NODE_ENV: z
