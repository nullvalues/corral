 ...
 *
 * Port pinning notes:
 * - `port: 6081` sits inside Corral Talent's reserved dev port range 6080–6089
 *   (see docs/brief.md → Constraints).
 * - `strictPort: true` is deliberate: if the port is taken, Vite must FAIL
 *   loudly instead of silently drifting to 6082. Port drift hides
 ...
 *     `http://localhost:6080` (Corral Talent's reserved API port). Used by the
